...
function MetricCard({ label, value, unit, icon, color, threshold }) {
   return (
      <div className="rounded-xl md:rounded-[2.5rem] p-3 md:p-8 bg-black/30 backdrop-blur-[80px] border border-white/10 transition-all hover:scale-105 group relative overflow-hidden">
         ...
         <div className="flex justify-between items-start mb-2 md:mb-6 relative z-10">
            <div className="p-2 md:p-4 rounded-xl md:rounded-2xl bg-white/10 shadow-inner border border-white/5">
               {React.cloneElement(icon, { size: window.innerWidth < 768 ? 16 : 28 })}
            </div>
            {threshold && <div className="text-[6px] md:text-[9px] font-black uppercase text-slate-400 bg-white/10 px-2 md:px-4 py-1 rounded-full border border-white/5">{threshold}</div>}
         </div>
...
export default function App() {
   ...
   client.on('offline', () => setIsOnline(false));
   client.on('error', () => setIsOnline(false));
   ...
   const testComp = async (c) => {
      ...
      setTimeout(() => setTestStatus(p => ({ ...p, [c]: 'success' })), 1000);
         ...
         <div className="min-h-screen text-slate-100 p-3 md:p-8 font-sans selection:bg-purple-500/30 relative z-10">
            <div className="max-w-[1400px] mx-auto space-y-6 md:space-y-6 relative">
               ...
               {/* ROW 1: HEADER */}
                <header className="flex flex-col md:flex-row justify-between items-center gap-6 bg-black/30 backdrop-blur-[80px] p-6 md:p-10 rounded-[2.5rem] md:rounded-[4rem] border border-white/10 shadow-2xl relative overflow-hidden w-full">
                  {/* Left Column: Agricultural Core Title & Big Digital Clock */}
                  <div className="flex flex-col gap-2 items-center md:items-start text-center md:text-left">
                     <h1 className="text-2xl md:text-4xl font-black italic uppercase tracking-tighter bg-gradient-to-r from-blue-400 via-purple-500 to-orange-500 bg-clip-text text-transparent filter drop-shadow-[0_0_20px_rgba(249,115,22,0.25)] select-none">
                        Adiwira Caraka <span className="text-white"></span>
                     </h1>
                     
                     {/* Premium Giant Digital Clock (iOS Lock Screen Style) */}
                     <div className="flex flex-col items-center md:items-start my-1 w-full">
                     ...
                     <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 md:gap-3">
                        <div className="flex items-center gap-2 px-4 py-1.5 bg-white/10 rounded-full border border-white/20 shadow-lg backdrop-blur-md">
                           <div className={`w-2 h-2 rounded-full ${isOnline ? 'bg-green-400 animate-pulse shadow-[0_0_8px_rgba(74,222,128,0.8)]' : 'bg-red-500'}`} />
                           <span className="text-[9px] md:text-[11px] font-black uppercase text-white/90">{isOnline ? 'System Online' : 'Core Offline'}</span>
                        </div>
                        
                        <div className="flex items-center gap-2 px-4 py-1.5 bg-white/10 rounded-full text-blue-300 text-[9px] md:text-[11px] font-bold border border-white/20 shadow-lg backdrop-blur-md">
                           <Globe size={12} className="animate-pulse" /> MQTT WSS CLOUD
                        </div>
                     </div>
                  </div>
                        ...
                        <>
                           ...
                           <div className="relative group shrink-0">
                              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-slate-900 border border-orange-500/30 flex items-center justify-center overflow-hidden shadow-2xl transition-all group-hover:border-orange-500 relative">
                                 {profile.avatar_url ? <img src={profile.avatar_url} className="w-full h-full object-cover" /> : <User size={18} className="text-white/60" />}
               ...
               {/* Removed IP Edit AnimatePresence */}
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
                  <div className="bg-black/30 backdrop-blur-[80px] p-6 md:p-8 rounded-[3rem] border border-white/10 shadow-none h-[220px] md:h-[320px] transition-all hover:border-orange-500/20">
                     <h3 className="text-[8px] md:text-[11px] font-black text-white/60 uppercase tracking-widest mb-4 flex items-center gap-3"><BarChart3 size={16} className="text-orange-500" /> Temp Wave</h3>
                  ...
                  <div className="bg-black/30 backdrop-blur-[80px] p-6 md:p-8 rounded-[3rem] border border-white/10 shadow-none h-[220px] md:h-[320px] transition-all hover:border-blue-500/20">
                     <h3 className="text-[8px] md:text-[11px] font-black text-white/60 uppercase tracking-widest mb-4 flex items-center gap-3"><BarChart3 size={16} className="text-blue-500" /> Hum Wave</h3>
                     <div className="h-[140px] md:h-[220px]">
                        <Line data={{
                           labels: history.labels,
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-10 items-stretch pb-20">
                  ...
                  <div className={`bg-black/30 backdrop-blur-[80px] p-6 md:p-8 rounded-[2.5rem] border border-white/10 shadow-none flex flex-col justify-between ${profile.role === 'pengunjung' ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
                     <div>
                        ...
                        <div className="mb-6">
                           <h3 className="text-[10px] md:text-[12px] font-black text-white/60 uppercase tracking-widest flex items-center gap-3 mb-6"><Settings size={18} className="text-orange-500" /> Parameters</h3>
                           <form onSubmit={async (e) => {
                              e.preventDefault();
                              publishCmd('update_settings', { tempMax: parseFloat(status.tempMax), tempMin: parseFloat(status.tempMin), humMin: parseFloat(status.humMin) });
                              alert("Command Synchronized via MQTT!");
                           }} className="space-y-6">
                              <div className="grid grid-cols-3 gap-4">
                                 {['tempMax', 'tempMin', 'humMin'].map(k => (
                                    <div key={k} className="space-y-1.5">
                                       <label className="text-[8px] md:text-[9px] font-black text-white/60 uppercase tracking-widest ml-1">{k}</label>
                                       <input type="number" step="any" className="w-full bg-white/10 p-3.5 rounded-xl text-white font-bold text-xs border border-white/5 focus:border-orange-500 outline-none" value={status[k]} onChange={e => setStatus({ ...status, [k]: e.target.value })} />
                                    </div>
                                 ))}
                              </div>
                        ...
                        <div className="mb-6">
                           <h3 className="text-[10px] md:text-[12px] font-black text-white/60 uppercase tracking-widest flex items-center gap-3 mb-6"><Power size={18} className="text-orange-500 animate-pulse" /> System Controls</h3>
                        ...
                        <div>
                           <div className="flex justify-between items-center mb-6">
                              <h3 className="text-[10px] md:text-[12px] font-black text-white/60 uppercase tracking-widest flex items-center gap-3"><Clock size={18} className="text-orange-500" /> History Logs</h3>
                              <button onClick={() => {
                  ...
                  <div className={`bg-black/30 backdrop-blur-[80px] p-6 md:p-8 rounded-[2.5rem] border border-white/10 shadow-none flex flex-col gap-6 justify-between ${profile.role !== 'admin' ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
                     <div>
                        ...
                        <div className="border-b border-white/5 pb-4">
                           ...
                           <span className="text-[7px] font-mono text-white/60 uppercase tracking-wider block mt-1">Live Hardware Telemetry Engine</span>
                        </div>
                        ...
                        <div className="border-t border-white/5 pt-4 mt-6">
                           <h4 className="text-[8px] md:text-[9px] font-black text-white/80 uppercase tracking-[0.2em] mb-4">Diagnostics Test Interface</h4>
                     ...
                     {profile.role === 'admin' && (
                        <div className="mt-6 border-t border-white/5 pt-6">
                           <h3 className="text-[10px] md:text-[12px] font-black text-white/60 uppercase tracking-widest flex items-center gap-3 mb-6"><Users size={18} className="text-blue-500" /> User Database</h3>
            ...
            <AnimatePresence>
               {showLogin && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-3xl p-6">
                     <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-[#1e293b] p-8 md:p-12 rounded-[3.5rem] md:rounded-[4rem] w-full max-w-sm border border-white/20 shadow-2xl relative animate-fadeIn">
                        <button onClick={() => { setShowLogin(false); setAuthMode('login'); setUsername(''); setFullname(''); setPassword(''); setSecretKey(''); }} className="absolute top-8 right-8 text-white/60 hover:text-white"><X /></button>

                        {/* Auth Tabs */}
                        <div className="flex bg-white/10 p-1.5 rounded-2xl border border-white/5 mb-8">
                           <button onClick={() => { setAuthMode('login'); setUsername(''); setFullname(''); setPassword(''); }} className={`flex-1 py-3 text-[10px] font-black uppercase tracking-wider rounded-xl transition-all ${authMode === 'login' ? 'bg-orange-600 text-white shadow-lg' : 'text-white/80 hover:text-white'}`}>
                              Sign In
                           </button>
                           <button onClick={() => { setAuthMode('register'); setUsername(''); setFullname(''); setPassword(''); }} className={`flex-1 py-3 text-[10px] font-black uppercase tracking-wider rounded-xl transition-all ${authMode === 'register' ? 'bg-orange-600 text-white shadow-lg' : 'text-white/80 hover:text-white'}`}>
                              Register
                           </button>
                        </div>
                        ...
                        <form onSubmit={async (e) => {
                           ...
                           if (authMode === 'login') {
                              ...
                              if (password.length < 6) {
                                 alert("Pendaftaran Gagal: Password harus terdiri dari minimal 6 karakter.");
                                 return;
                              }
                           ...
                           {authMode === 'register' && (
                              <input type="text" placeholder="Nama Lengkap" className="w-full bg-white/10 p-4 rounded-2xl text-white outline-none border border-white/20 shadow-inner text-xs focus:border-orange-500" value={fullname} onChange={e => setFullname(e.target.value)} required />
                           )}

                           <input type="text" placeholder="Username" className="w-full bg-white/10 p-4 rounded-2xl text-white outline-none border border-white/20 shadow-inner text-xs focus:border-orange-500" value={username} onChange={e => setUsername(e.target.value)} required />
                           <input type="password" placeholder="Password" className="w-full bg-white/10 p-4 rounded-2xl text-white outline-none border border-white/20 shadow-inner text-xs focus:border-orange-500" value={password} onChange={e => setPassword(e.target.value)} required />
                              ...
                              <div className="space-y-4 text-left animate-fadeIn">
                                 <div className="space-y-2">
                                    ...
                                    <div className="flex bg-slate-900/50 p-1.5 rounded-xl border border-white/5 gap-2">
                                       <button type="button" onClick={() => setRole('anggota')} className={`flex-1 py-2 text-[8px] font-bold uppercase rounded-lg transition-all ${role === 'anggota' ? 'bg-blue-600/30 text-blue-400 border border-blue-500/20' : 'text-white/60'}`}>Anggota</button>
                                       <button type="button" onClick={() => setRole('admin')} className={`flex-1 py-2 text-[8px] font-bold uppercase rounded-lg transition-all ${role === 'admin' ? 'bg-orange-600/30 text-orange-400 border border-orange-500/20' : 'text-white/60'}`}>Admin</button>
                                    </div>
                                 </div>
                                 <div className="space-y-2">
                                    <label className="text-[8px] font-black text-white/60 uppercase tracking-widest ml-1">Registration Key</label>
                                    <input type="password" placeholder="Secret Key" className="w-full bg-white/10 p-4 rounded-2xl text-white outline-none border border-white/20 shadow-inner text-xs focus:border-orange-500" value={secretKey} onChange={e => setSecretKey(e.target.value)} required />
                                 </div>
                              </div>
                           )}
            ...
            <AnimatePresence>
               {showProfileModal && user && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-3xl p-6">
                     <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-[#1e293b] p-8 md:p-12 rounded-[3.5rem] md:rounded-[4rem] w-full max-w-sm border border-white/20 shadow-2xl relative animate-fadeIn">
                        <button onClick={() => { setShowProfileModal(false); setNewPassword(''); setConfirmPassword(''); }} className="absolute top-8 right-8 text-white/60 hover:text-white"><X /></button>
                        ...
                        <form onSubmit={async (e) => {
                           ...
                           <div className="flex flex-col items-center gap-3">
                              ...
                              <span className="text-[7px] font-black uppercase text-white/60 tracking-wider">Tap to Change Avatar</span>
                           </div>

                           <div className="space-y-3 text-left">
                              <div className="space-y-1.5">
                                 <label className="text-[8px] font-black text-white/60 uppercase tracking-widest ml-1">Nama Lengkap</label>
                                 <input type="text" className="w-full bg-white/10 p-3.5 rounded-xl text-white outline-none border border-white/20 shadow-inner text-xs focus:border-blue-500" value={newFullname} onChange={e => setNewFullname(e.target.value)} required />
                              </div>

                              <div className="space-y-1.5">
                                 <label className="text-[8px] font-black text-white/60 uppercase tracking-widest ml-1">Username</label>
                                 <input type="text" className="w-full bg-white/10 p-3.5 rounded-xl text-white outline-none border border-white/20 shadow-inner text-xs focus:border-blue-500 font-mono" value={newUsername} onChange={e => setNewUsername(e.target.value)} required />
                              </div>

                              <div className="space-y-1.5">
                                 <label className="text-[8px] font-black text-white/60 uppercase tracking-widest ml-1">New Password (Optional)</label>
                                 <input type="password" placeholder="Biarkan kosong jika tidak diubah" className="w-full bg-white/10 p-3.5 rounded-xl text-white outline-none border border-white/20 shadow-inner text-xs focus:border-blue-500" value={newPassword} onChange={e => setNewPassword(e.target.value)} />
                              </div>

                              {newPassword && (
                                 <div className="space-y-1.5 animate-fadeIn">
                                    <label className="text-[8px] font-black text-white/60 uppercase tracking-widest ml-1">Confirm New Password</label>
                                    <input type="password" placeholder="Ulangi password baru" className="w-full bg-white/10 p-3.5 rounded-xl text-white outline-none border border-white/20 shadow-inner text-xs focus:border-blue-500" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required />
                                 </div>
                              )}
                           </div>
